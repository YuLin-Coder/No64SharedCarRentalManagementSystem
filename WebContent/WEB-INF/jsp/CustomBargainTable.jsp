...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/bargainAddCustom");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("bargain/informationLoadCustomOne?id="+id);
	        }
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${bargainList.datas }" var="bargain" varStatus="vg">
						<tr>
							<td><c:out value="${bargain['code']}"></c:out></td>
							<td><c:out value="${bargain['userInfo']['userName']}"></c:out></td>
							<td><c:out value="${bargain['car']['name']}"></c:out></td>
							<td><c:out value="${bargain['car']['price']}"></c:out></td>
							<td><c:out value="${bargain['price']}"></c:out></td>
							<td>
								<c:if test="${bargain['status'] eq 0}">
									<c:out value="租赁中" />
								</c:if>
								<c:if test="${bargain['status'] eq 1}">
									<c:out value="已归还" />
								</c:if>
							</td>
							<td><c:out value="${bargain['createtime']}"></c:out></td>
							<td>
								<c:if test="${bargain['status'] eq 0}">
									<c:out value="暂未归还" />
								</c:if>
								<c:if test="${bargain['status'] eq 1}">
									<c:out value="${bargain['return_time'] }" />
								</c:if>
							</td>
							<td>
							<c:if test="${bargain['status'] eq 0}">
							<a href="bargain/back?id=${bargain['id'] }" class="tablelink">归还</a> 
							</c:if>
							<a href="javascript:;"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="bargain/informationLoadCustom?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="bargain/informationLoadCustom?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="bargain/informationLoadCustom?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="bargain/informationLoadCustom?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="bargain/informationLoadCustom?pageNow=${count }">${count}</a></li>
