...
<link href="css/css/background/css/select.css" rel="stylesheet"
	type="text/css" />
<link href="css/css/background/css2/table_image.css" rel="stylesheet" type="text/css"/>
...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/carAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("car/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="car/informationLoad" method="POST"
		onsubmit="return check();">
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>车牌号码</label><input type="text" class="scinput1"
						name="code" value="${car['code'] }"></li>
		...
		<div class="rightinfo">
			...
			<table class="imgtable">
				...
				<tbody>
					...
					<c:forEach items="${carList.datas }" var="car" varStatus="vg">
						<tr>
							<td>
								<div id="images">
								<img src="upload/upload/${car['url'] }">
								</div>
							</td>
							<td><c:out value="${car['name']}"></c:out></td>
							<td><c:out value="${car['code']}"></c:out></td>
							<td><c:out value="${car['type']['name']}"></c:out></td>
							<td><c:out value="${car['factory']}"></c:out></td>
							<td><c:out value="${car['stand']}"></c:out></td>
							<td><c:out value="${car['address']}"></c:out></td>
							...
							<td><c:out value="${car['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${car['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${car['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="car/informationLoad?pageNow=${pager.pageNow }&code=${car['code']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="car/informationLoad?pageNow=${pager.pageNow-1 }&code=${car['code']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="car/informationLoad?pageNow=${vs.count }&code=${car['code']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="car/informationLoad?pageNow=${count }&code=${car['code']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="car/informationLoad?pageNow=${count }&code=${car['code']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="car/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
