...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/customAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("custom/informationLoadOne?id="+id);
	        }
				...
				<tbody>
					...
					<c:forEach items="${customList.datas }" var="custom" varStatus="vg">
						<tr>
							...
							<td><c:out value="${custom['address']}"></c:out></td>
							<td><c:out value="${custom['pledge']}"></c:out></td>
							<td><c:out value="${custom['card']}"></c:out></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="custom/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="custom/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="custom/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="custom/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="custom/informationLoad?pageNow=${count }">${count}</a></li>
